...
const Quiz = () => {
  ...
  const addAnswer = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    localStorage.setItem('quiz', JSON.stringify(latestAnswers)); //localStorage에 넣을때는 항상 string으로 변경해서 넣어라
  };
  ...
  return (
    <>
      ...
      <div className={styles.navBtns}>
        {prev ? <button onClick={() => setPageIndex(pageIndex - 1)}>이전 문제</button> : <Link href='/'>취소</Link>}
        {next ? (
          <button
            onClick={() => setPageIndex(pageIndex + 1)}
            disabled={answered[quiz.id] === undefined}
            className={answered[quiz.id] === undefined ? 'disabledBtn' : 'activeBtn'}
          >
            다음 문제
          </button>
        ) : (
